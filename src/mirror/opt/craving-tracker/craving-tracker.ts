...
const STATE_TITLES = [
  "Tidak ada hasrat / Lupa",
  "Ada rasa ingin, langsung hilang",
  "Ingin banget",
  "Jebol",
];
...
const HEAD_BG = [
  "rgba(155,233,168,0.14)",
  "rgba(86,211,100,0.16)",
  "rgba(26,127,55,0.28)",
  "rgba(255,45,45,0.21)",
];
...
function placeholderB64(level: number): string {
  ...
  const svg =
    ...
    '<text x="240" y="146" fill="#4e5d78" font-family="Consolas, monospace" font-size="13" text-anchor="middle">' + file + ' &#8594; /opt/craving-tracker/</text>' +
...
function brokenB64(file: string): string {
  const svg =
    ...
    '<text x="240" y="146" fill="#a06a6a" font-family="Consolas, monospace" font-size="13" text-anchor="middle">' + file + "</text>" +
...
async function loadLevelImage(
  level: number,
): Promise<{ img: ImageResult | null; corrupt: string | null }> {
...
export const main = Program(async (_args: string[]) => {
  ...
  const headerRow = HStack(
    { gap: "8px", width: "100%", alignItems: "baseline" },
    lblTitle,
    lblEncourage,
  );
  form.add(headerRow);
  ...
  const statsRow = HStack(
    { gap: "8px", width: "100%", alignItems: "center" },
    statsHost,
    btnClear,
  );
    ...
    for (let col = 0; col < weeks; col++) {
      ...
      for (let row = 0; row < 7; row++) {
        ...
        const maxState = count
          ? Math.max.apply(null, list.map((x) => x.state))
          : -1;
        ...
        const base =
          count === 0 ? GH_EMPTY : STATE_HEX[maxState < 0 ? 0 : maxState];
  ...
  async function renderCalendar(screen: any): Promise<void> {
    ...
    await writeText(
      activityHost,
      "📊 " + cal.total + " log " + WINDOW_LABEL + "   ·   " +
        cal.activeDays + " hari aktif   ·   🔥 " + cal.jebol +
        " jebol   ·   streak terpanjang " + cal.longest + " hari",
      {},
      screen,
    );
  ...
  async function renderTrend(screen: any): Promise<void> {
    ...
    for (let i = 0; i < recs.length; i += 7) {
      ...
      const avg = avgOf(a);
      const tip =
        "Minggu " + fmtDateLong(wk[0].d) + ": " +
        (a.days === 0
          ? "belum ada log"
          : a.days + " hari ber-log · avg " + avg.toFixed(2) +
            " · tenang " + calmPct(a) + "%" +
      ...
      if (a.jebol > 0) {
        shadows.push(
          "0 0 " + Math.min(4 + a.jebol * 4, 16) + "px " +
          Math.min(1 + a.jebol, 3) + "px " +
      ...
      bars.push({
        ...
        props: {
          ...
          style: {
            ...
            minWidth: "3px",
            height:
              (a.days === 0 ? 3 : Math.round(8 + (avg / 3) * (H - 8))) + "px",
            borderRadius: "2px",
            background:
              a.days === 0
                ? "rgba(255,255,255,0.07)"
                : STATE_HEX[Math.min(3, Math.round(avg))],
    ...
    if (prev.days < MIN_DAYS || cur.days < MIN_DAYS) {
      await writeText(
        trendHeadHost,
        "❔ Belum bisa dinilai — butuh ≥" + MIN_DAYS +
          " hari ber-log di tiap periode",
        { color: "var(--text-muted)" },
        screen,
      );
      await writeText(
        trendPrevHost,
        "3 bln lalu: " + prev.days + " hari ber-log",
        {},
        screen,
      );
      await writeText(
        trendCurHost,
        "3 bln kini: " + cur.days + " hari ber-log",
        {},
        screen,
      );
      return;
    ...
    await writeText(
      trendHeadHost,
      verdict + " — avg hasrat " + avgOf(cur).toFixed(2) + " (" +
        (dAvg <= 0 ? "turun " : "naik ") + Math.abs(dAvg).toFixed(2) +
    ...
    await writeText(
      trendPrevHost,
      "3 bln lalu: avg " + avgOf(prev).toFixed(2) +
        " · tenang " + calmPct(prev) + "% · " + prev.jebol + " jebol" +
        "  (" + prev.days + " hari)",
      {},
      screen,
    );
    await writeText(
      trendCurHost,
      "3 bln kini: avg " + avgOf(cur).toFixed(2) +
        " · tenang " + calmPct(cur) + "% · " + cur.jebol + " jebol" +
        "  (" + cur.days + " hari, " + (dCalm >= 0 ? "+" : "") + dCalm +
        " poin)",
      {},
      screen,
    );
    ...
    if (todayLogs.length === 0) {
      ...
      if (jebol > 0) {
        if (jebol >= 10) {
          ...
          const v = [
            "🛑 " + jebol + "× jebol hari ini. Niat saja tidak cukup — ganti pemicunya (kopi, duduk lama, koding tengah malam).",
            "🛑 " + jebol + "× jebol hari ini. Berhenti total butuh strategi baru, bukan cuma tekad.",
            "🛑 " + jebol + "× jebol dalam sehari. Ini sinyal kuat: minta bantuan orang terdekat / layanan berhenti merokok.",
          ];
          msg = v[variantIndex(v.length)];
          color = STATE_HEX[3];
        } else if (jebol >= 6) {
  ...
  async function renderStats(screen: any): Promise<void> {
    ...
    await writeText(
      statsHost,
      "Σ " + total + " entri   ·   " +
        counts[0] + " lupa · " + counts[1] + " reda · " +
        counts[2] + " kuat · " + counts[3] + " jebol" +
  ...
  function applyLevelImage(
    lv: number,
    res: { img: ImageResult | null; corrupt: string | null },
  ): void {
    if (res.img) {
      imgChild.setBase64(res.img.b64, res.img.mime);
      lblImgInfo.caption =
        "🖼 " + res.img.file + "  (" + fmtKb(res.img.bytes) + ")";
      return;
    }
    if (res.corrupt) {
      imgChild.setBase64(brokenB64(res.corrupt), "image/svg+xml");
      lblImgInfo.caption =
        "⚠️ " + res.corrupt + " rusak (byte korup) — kirim ulang via `npm run install`";
      return;
    }
    imgChild.setBase64(placeholderB64(lv), "image/svg+xml");
    lblImgInfo.caption =
      "⚠️ " + APP_DIR + "/level" + lv + ".jpg belum ada — taruh 4 foto di folder itu";
  }
  ...
  form.onSetup = async (screen) => {
    ...
    cbLevel.onChange = async (idx: number) => {
      ...
      await writeStatus(
        screen,
        "left",
        "Level " + level + " — " + STATE_LABELS[level],
      );
    };
    ...
    await writeStatus(
      screen,
      "left",
      "📄 " + DATA_FILE + "   ·   pilih skala 0-3 untuk melihat foto",
    );
  };
  ...
  btnSubmit.onClick = async () => {
    ...
    const ctx =
      (edContext.text || "").trim() || "Koding santai / Refactoring biasa";
    const ts = Date.now();
    ...
    await writeStatus(
      form.screen,
      "left",
      ok ? "✅ Tersimpan → " + DATA_FILE : "❌ Gagal menulis " + DATA_FILE,
    );
    if (state === 3) {
      await writeStatus(
        form.screen,
        "right",
        "Jebol tercatat — besok mulai lagi dari 0 💪",
      );
    }
  };
  ...
  btnClear.onClick = async () => {
    ...
    await writeStatus(
      ...
      "left",
      ok
        ? "🗑 Data dibersihkan → " + DATA_FILE
        : "❌ Gagal menulis " + DATA_FILE,
    );
  };
