 ...
 *   relay  @0x20  → /dev/relays (atau /dev/mcp0, /dev/mcp23017)
 *   saklar @0x24  → /dev/switches (atau /dev/mcpSw, /dev/mcp1)
 *
...
export const main = Program(async (args: string[]) => {
  ...
  if (relayFd) {
    for (let p = 0; p < 16; p++) {
      ...
      try {
        await fs.ioctl(relayFd, IOCTL_SET_PIN_MODE, { pin, mode: MODE_OUTPUT });
      } catch (_) {
        /* ignore */
      }
    }
  ...
  async function writeRelayPort(port: number, on: boolean): Promise<boolean> {
    if (relayFd === null) return false;
    try {
      const pin = portToPin(port);
      const value = on ? 0 : 1;
      return (
        (await fs.ioctl(relayFd, IOCTL_DIGITAL_WRITE, { pin, value })) === true
      );
    } catch (_) {
      return false;
    }
  ...
  function pushState(toPid?: number) {
    ...
    if (toPid !== undefined) {
      ...
      for (const p of subscribers) void shell.send(p, data).catch(() => {});
    }
  }
  ...
  lib?.onEvent?.("ipc_message", (msg: any) => {
    ...
    if (!p || !p.type) return;
    const fromPid = typeof msg?.fromPid === "number" ? msg.fromPid : undefined;
  ...
  setInterval(() => {
    void updateSwitchAndLamp().catch(() => {});
  }, POLL_MS);
