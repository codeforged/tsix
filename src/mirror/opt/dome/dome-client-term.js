...
(function () {
    ...
    //                   (warnanya dari crt.specular — ikut profil tabung)
    //   [3] Vignette   : tepi tabung menggelap
    //   [4] Scanlines  : garis horizontal periodik
    //   [5] Tint       : warna fosfor tipis (hijau/amber; kosong = warna asli)
    ...
    function applyCrtFx(el, crt) {
        el.querySelectorAll("._tsix_crt").forEach(function (n) {
            n.remove();
        });
        ...
        el.style.boxShadow =
            "inset 0 0 44px 12px rgba(0,0,0," +
            (edge * 0.62).toFixed(3) +
            ")," +
            "inset 0 0 120px 30px rgba(0,0,0," +
            (edge * 0.42).toFixed(3) +
            ")";
        ...
        // Warna kilau diatur app lewat `crt.specular` ({ top, bottom } = "r,g,b"),
        // supaya ikut profil tabung (hijau / amber / berwarna). Default = hijau,
        // yaitu perilaku lama RetroTerm sebelum ada profil.
        var sp = crt.specular || {};
        var spTop = sp.top || "190,255,210";
        var spBottom = sp.bottom || "120,255,170";
        layers.push(
            "radial-gradient(ellipse 130% 100% at 28% 8%, rgba(" +
                spTop +
                "," +
                hi.toFixed(3) +
                "), rgba(0,0,0,0) 58%)",
        ...
        layers.push(
            "radial-gradient(ellipse 110% 85% at 74% 97%, rgba(" +
                spBottom +
                "," +
                (hi * 0.5).toFixed(3) +
                "), rgba(0,0,0,0) 46%)",
        ...
        layers.push(
            "radial-gradient(ellipse at center, rgba(0,0,0,0) 52%, rgba(0,0,0," +
                (crt.vignette != null ? crt.vignette : 0.45) +
                ") 100%)",
        );
        // [4] Scanlines.
        layers.push(
            "repeating-linear-gradient(0deg, rgba(0,0,0," +
                ...
                "rgba(0,0,0," +
                lineAlpha +
                ") 1px, rgba(0,0,0,0) 1px, rgba(0,0,0,0) " +
                period +
                "px)",
        );
        // [5] Tint fosfor.
        ...
        if (crt.flicker) {
            ...
            anim.textContent =
                "@keyframes " +
                key +
                " {0%,100%{opacity:.978}50%{opacity:1}}" +
                '[data-tsix-id="' +
                CSS.escape(el._xtermNodeId || "") +
                '"] ._tsix_crt_fx' +
                "{animation:" +
                key +
                " 120ms steps(2,end) infinite;}";
            el.appendChild(anim);
        }
    }
    ...
    function initXterm(el, themeColors, crtOptions) {
        ...
        const term = new Terminal({
            ...
            fontFamily: (_crt && _crt.fontFamily) || "'SF Mono', 'Menlo', 'Courier New', monospace",
        ...
        if (tt) {
            ...
            const selBase = '[data-tsix-id="' + CSS.escape(el._xtermNodeId || "") + '"]';
            style.textContent =
                selBase +
                ...
                var ready =
                    document.fonts && document.fonts.check
                        ? document.fonts.check(
                              _fontSize + "px '" + (_crt.fontFamily || "").split(",")[0].replace(/['"]/g, "") + "'",
                          )
                        : true;
