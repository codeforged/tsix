...
export const main = Program(async (args: string[]) => {
    ...
    try {
        ...
        if (raw) {
            ...
            await std.log(
                `[retroterm] Bezel dimuat dari ${BEZEL_PATH} (${raw.length} byte)`,
                "retroterm",
            );
        } else {
            await std.log(
    ...
    // Angka inset mengikuti area layar pada retro-crt.jpg (diukur dari gambar:
    // layar kira-kira x 13-79%, y 13-78%).
    const crtTheme = {
        enabled: true,
        ...
        bezel: {
            imageUrl: bezelUrl,
            // Area layar di dalam gambar (persen) — dipakai untuk memotong bezel
            // supaya hanya casing-nya yang tampak.
            holeTop: "13%",
            holeBottom: "78%",
            holeLeft: "13%",
            holeRight: "79%",
            screenRadius: "20px",
            inset: {
                // Padding window → area layar. Dikompensasi supaya teks tidak
                // tertutup casing pada ukuran window default.
                top: "15%",
                bottom: "24%",
                left: "15%",
                right: "23%",
