...
export default class TSSHClient {
  async execute(lib: UserLib, args: string[]) {
    ...
    try {
      // --- PHASE 1: HANDSHAKE ---
      const reqPkt = TSSHProtocol.pack(
        TSSHOpcode.HANDSHAKE_REQ,
        TSSHChannel.CONTROL,
      );
      ...
      const resp = await this.waitForOpcode(
        lib,
        fd,
      ...
      const [serverPubKey, serverFingerprint] = resp.payload
        .toString("utf8")
        .split("::");
      ...
      // Verify Known Hosts
      const verified = await this.verifyHost(
        lib,
        remoteAddr,
        serverFingerprint,
      );
      ...
      const encryptedKeyHex = SecurityAgent.encryptWithPublicKey(
        serverPubKey,
        sessionKey,
      );

      const kexPkt = TSSHProtocol.pack(
        TSSHOpcode.KEY_EXCHANGE,
        TSSHChannel.CONTROL,
        encryptedKeyHex,
      );
      await lib.net.sendto(
      ...
      const ack = await this.waitForOpcode(
        lib,
        fd,
      ...
      // (per-session key), BUKAN oleh driver. tsshd melayani banyak session
      // di SATU port dengan key berbeda-beda — model security driver
      // per-port (1 key/port) tidak cukup. Jadi driver dibiarkan plain
      // (jangan upgrade ioctl 0x1001), hanya framing biner-nya pakai Binfeo.
      ...
      const remoteCommand =
        cmdIdx !== -1 && cmdIdx + 1 < args.length ? args[cmdIdx + 1] : "";

      const encryptedCmd = agent.securePacketOut(remoteCommand);
      const connReq = TSSHProtocol.pack(
        TSSHOpcode.CONNECT_REQ,
        TSSHChannel.CONTROL,
        encryptedCmd,
      );
      await lib.net.sendto(
      ...
      await lib.shell.onSignal("SIGINT", async () => {
        ...
        const intMsg = TSSHProtocol.pack(
          TSSHOpcode.DATA,
          TSSHChannel.SHELL,
          encryptedInt,
        );
          ...
          if (rawPkt && rawPkt.data) {
            ...
            if (pkt) {
              if (pkt.opcode === TSSHOpcode.DATA) {
                const decrypted = agent.securePacketIn(
      ...
      const keepAlive = async () => {
        while (active) {
          ...
          if (!active) break;
          const pingPkt = TSSHProtocol.pack(
            TSSHOpcode.PING,
            TSSHChannel.CONTROL,
          );
          try {
            ...
            /* koneksi mati — bridgeOut akan menangani */
          }
        }
      };
  ...
  private async waitForOpcode(
    lib: UserLib,
    fd: number,
    expectedOpcode: TSSHOpcode,
    timeout: number,
  ) {
  ...
  private async verifyHost(
    ...
    let content = "";
    try {
      content = (await lib.fs.readFile(knownHostsPath)) || "";
    } catch (_) {}
    ...
    try {
      await lib.fs.mkdir(`${home}/.ssh`);
    } catch (_) {}
    await lib.fs.writeFile(
      knownHostsPath,
      content + (content ? "\n" : "") + `${host} ${fingerprint}\n`,
    );
    return true;
