...
export const main = Program(async (_args: string[]) => {
    ...
    body.add(HStack({ justifyContent: "space-between", width: "100%" }, lblPinLeft, lblPinRight));
    ...
    const makeBtn = (id: string, caption: string, onClick: () => void, width = "104px"): TButton => {
    ...
    form.onSetup = async (screen) => {
        ...
        await std.log(`[plcd-emulator] device=${info?.device} framebufferSize=${info?.framebufferSize}`);
        ...
        anim = await mountDDC(screen, { id: "ddc-plcd", source: src, width: PHYS_W, height: PHYS_H }, "stage");
