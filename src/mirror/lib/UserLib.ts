...
export class StdLib {
    ...
    constructor(private dispatch: (code: SyscallCode, args: any) => Promise<any>) {}
        ...
        try {
            ...
            if (stack) {
                ...
                for (const l of lines) {
                    ...
                    if (p.includes("UserLib") || p.includes("Application") || p.includes("emerald")) continue;
...
export class FsLib {
    ...
    public async syncFromHost(hostPath: string, vfsPath: string): Promise<boolean> {
    ...
    public async getMounts(): Promise<{ vfsPath: string; type: string; source: string; readOnly: boolean }[]> {
    ...
    public async copyWithProgress(
        ...
        const firstChunk = await this.readChunk(srcPath, 0, Math.min(step, totalSize));
        ...
        while (offset < totalSize) {
            const readLen = Math.min(step, totalSize - offset);
...
export class WebLib {
    ...
    private ensureDispatch(): void {
        ...
        this.lib.onEvent("http_event", (p: any) => {
            ...
            else if (p.type === "LISTENING") this.fire("listening", { ...p, source: "http" });
            else if (p.type === "LISTEN_ERROR") this.fire("error", { ...p, source: "http" });
        });
        this.lib.onEvent("ws_event", (p: any) => {
            ...
            else if (p.type === "LISTENING") this.fire("listening", { ...p, source: "ws" });
            else if (p.type === "LISTEN_ERROR") this.fire("error", { ...p, source: "ws" });
        });
    }
    ...
    private waitChannel(channel: string, timeoutMs: number = 5000): Promise<{ ok: boolean; error?: string }> {
        return new Promise((resolve) => {
            ...
            const h = (p: any) => {
                ...
                else if (p.type === "LISTEN_ERROR") finish({ ok: false, error: p.message || "listen failed" });
            };
            this.lib.onEvent(channel, h);
            setTimeout(() => finish({ ok: false, error: `timeout ${timeoutMs}ms (${channel})` }), timeoutMs);
        });
    }
    ...
    public async start(
        ...
        if (wantsHttp) {
            ...
            if (fd === null) return { ok: false, error: "/dev/httpd tidak tersedia", port };
        ...
        if (wantsWs) {
            ...
            if (fd === null) return { ok: false, error: "/dev/wsd tidak tersedia", port };
            if (wantsHttp) {
...
export class ShellLib {
    ...
    public async daemonize(serviceName?: string): Promise<boolean> {
        ...
        if (ok) {
            ...
            await (this as any)._lib.std.log(`${name} (${this.pid}) started in background.`, "system");
        }
        return ok;
    }
