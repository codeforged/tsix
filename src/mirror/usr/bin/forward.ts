...
export default class main {
    async execute(lib: UserLib, args: string[]) {
        ...
        const defaultBroker = cfg.network.interfaces?.[0]?.broker || "mqtt://localhost";
        ...
        if (hasStop) {
            ...
            await lib.std.print(`Stopping broker bridge (${stats.brokerA} <-> ${stats.brokerB})...\n`);

            const success = await globalForwarder.stopForward();
            if (success) {
                await lib.std.print(`✅ Bridge stopped. Stats: A->B: ${stats.packetsAtoB}, B->A: ${stats.packetsBtoA}\n`);
                globalForwarder = null;
                try { await lib.fs.unlink(statsFile); } catch (e) { }
            } else {
        ...
        if (hasStart) {
            if (!globalForwarder) {
                await lib.std.print("No bridge configured. Use: forward -s <broker1> -d <broker2>\n");
                await lib.shell.exit(1);
                return;
            }
            ...
            const stats = globalForwarder.getStats();
            if (stats.isRunning) {
                await lib.std.print(`Bridge is already running (${stats.brokerA} <-> ${stats.brokerB})\n`);
                await lib.shell.exit(0);
                return;
            }

            await lib.std.print(`Resuming broker bridge (${stats.brokerA} <-> ${stats.brokerB})...\n`);
        ...
        if (sourceIdx !== -1 && destIdx !== -1) {
            ...
            if (globalForwarder) {
                const stats = globalForwarder.getStats();
                await lib.std.print(`A broker bridge is already running between ${stats.brokerA} and ${stats.brokerB}\n`);
                await lib.shell.exit(1);
                return;
            }

            await lib.std.print(`Establishing Broker Bridge: ${brokerA} <-> ${brokerB}\n`);
            ...
            if (success) {
                ...
                const stats = globalForwarder.getStats();
                if (stats.isRunning) {
                    await lib.std.print(`✅ Broker Bridge is ACTIVE between ${stats.brokerA} and ${stats.brokerB}\n`);
                    await lib.std.print("Running in background. Use 'forward -top' to monitor.\n");

                    if (await lib.shell.daemonize("Broker Bridge")) {
                        await lib.std.log(`Bridge established: ${stats.brokerA} <-> ${stats.brokerB}`, "forward");
                    }
                    ...
                    await lib.std.print("⚠️  Bridge started but failed to connect to one or both brokers.\n");
                }
            } else {
    ...
    private async runMonitor(lib: UserLib, statsFile: string) {
        ...
        while (!interrupted) {
            ...
            if (!dataStr) {
                ...
                await lib.std.print("Waiting for forwarder stats... (Is the bridge running?)\n");
                lastLines = 1;
                await new Promise(r => setTimeout(r, 1000));
                continue;
            }
            ...
            try {
                ...
                if (prevStats) {
                    ...
                    if (dt > 0.1) { // Min 100ms for calculation
                        rateAtoB = (stats.bytesAtoB - prevStats.bytesAtoB) / dt;
                        rateBtoA = (stats.bytesBtoA - prevStats.bytesBtoA) / dt;
                    }
                }
                prevStats = { bytesAtoB: stats.bytesAtoB, bytesBtoA: stats.bytesBtoA, time: now };
    ...
    private async showSyntax(lib: UserLib) {
        ...
        await lib.std.print("        forward -top      # Monitor real-time stats\n");
