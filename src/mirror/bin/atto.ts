...
class SimpleTextEditor {
  ...
  private lastSelectionStart: { line: number; col: number } | null = null;
  private lastSelectionEnd: { line: number; col: number } | null = null;
  ...
  public async init() {
    ...
    const envColumns = parseInt(
      (await this.os.shell.getenv("COLUMNS")) || "80",
    );

    const screen = (await this.os.std.getScreenInfo()) || {
      lines: envLines,
      columns: envColumns,
    };
    ...
    this.os.shell.onSignal("SIGWINCH", async () => {
      ...
      const newScreen = (await this.os.std.getScreenInfo()) || {
        lines: envLines,
        columns: envColumns,
      };
  ...
  private highlightLine(
    ...
    if (this.lang !== "typescript" && this.lang !== "javascript")
    ...
    try {
      ...
      if (fd !== null) {
        ...
        if (raw) {
          ...
          if (lc) {
            if (Array.isArray(lc.keywords))
              this.langKeywords = new Set(lc.keywords.map(String));
            if (Array.isArray(lc.builtins))
              this.langBuiltins = new Set(lc.builtins.map(String));
          }
  ...
  private renderLineContent(
    ...
    if (sel) {
      ...
      if (s < e) {
        const before = this.highlightLine(
          line,
          blockStarts[lineIdx] || false,
          sliceStart,
          s,
        );
        const selected = this.highlightLine(
          line,
          blockStarts[lineIdx] || false,
          s,
          e,
        );
        const after = this.highlightLine(
          line,
          blockStarts[lineIdx] || false,
          e,
          sliceEnd,
        ...
        text =
          before +
          selStart +
          selected.replace(/\x1b\[0m/g, "\x1b[0m" + selStart) +
          selEnd +
          after;
      } else {
        text = this.highlightLine(
          line,
          blockStarts[lineIdx] || false,
          sliceStart,
          sliceEnd,
  ...
  /**
   * Mengembalikan teks terseleksi dalam editor.
   */
  private getSelectionText(): string {
    if (this.selectionActive && this.selectionStart && this.selectionEnd) {
      ...
      if (lines.length === 1) {
        return lines[0].slice(s.col, e.col);
      } else {
  ...
  private finalizeSelection(): void {
    if (this.selectionActive) {
      this.clipboard = this.getSelectionText();
  ...
  public async renderStatusBar() {
    if (this.findMode) {
      ...
      if (this.findStep === "find") prompt = `Find: ${this.findText}${info}`;
        ...
        prompt = `Replace with: ${this.replaceText}${info}`;
      else if (this.findStep === "confirm")
        prompt = `Replace? (y)es (n)o (a)ll (q)uit${info}`;

      const padding = " ".repeat(Math.max(0, this.screenWidth - prompt.length));
      const sb = this.syntaxTheme.statusBar;
      await this.write(
        `\x1b[${this.screenHeight + 1};1H\x1b[${sb.fg}m\x1b[${sb.bg}m${prompt}${padding}\x1b[0m`,
      );
  ...
  private async refreshScreenSize(): Promise<boolean> {
    try {
      ...
      const s = (await this.os.std.getScreenInfo()) || {
        lines: envLines,
        columns: envColumns,
      };
  ...
  private async positionCursorInternal() {
    if (this.findMode) {
      ...
      if (this.findStep === "find") cursorCol = 7 + this.findText.length;
        ...
        cursorCol = 15 + this.replaceText.length;
      else if (this.findStep === "confirm") {
        const res = this.findResults[this.findCurrentIndex];
        cursorCol =
          Math.max(0, Math.min(this.screenWidth - 1, res.col - this.offsetX)) +
          1;
    ...
    const displayCursorX = Math.max(
      0,
      Math.min(this.textWidth, this.cursorX - this.offsetX),
    );
    // Gutter (numWidth) + separator (1 spasi) + posisi teks
    await this.write(
      `\x1b[${this.cursorY + 1};${this.numWidth + 2 + displayCursorX}H\x1B[?25h`,
    );
  }
  /**
   * INPUT HANDLING
  ...
  public async handleKey(char: string): Promise<string | void> {
    ...
    if (char === "\u001b") {
      ...
      // if (seq1 == "b") {
      //   await this.jumpToPrevWord();
      //   return;
      // }
      // if (seq1 == "f") {
      //   await this.jumpToNextWord();
      //   return;
      // }
      ...
      if (seq1 === "[") {
        ...
        // Mematikan visual selection ketika kursor dipindahkan tanpa Shift,
        // namun menyimpan koordinat range selection terakhirnya untuk Ctrl+X
        const finalizeOnNav = async () => {
          if (this.selectionActive) {
        ...
        if (seq2 === "3") {
          ...
          } else if (this.lastSelectionStart && this.lastSelectionEnd) {
            this.selectionActive = true;
            this.selectionStart = this.lastSelectionStart;
            this.selectionEnd = this.lastSelectionEnd;
            await this.deleteSelection();
        ...
        if (seq2 === "2") {
          await std.getChar();
          return;
        } // Insert \x1b[2~ (belum ada mode insert)
        if (seq2 === "4") {
          await std.getChar();
          await finalizeOnNav();
          await this.goEnd(false);
        ...
        if (seq2 === "Z") {
          ...
          return;
        } // Shift+Tab \x1b[Z → unindent baris terseleksi
          ...
          if (seq3 === ";") {
            ...
            if (seq4 === "2") {
              ...
              if (!this.selectionActive) {
                this.selectionActive = true;
                this.selectionStart = {
                  line: this.cursorY + this.offsetY,
                  col: this.cursorX,
                };
                this.lastSelectionStart = null;
                this.lastSelectionEnd = null;
              ...
              if (seq5 === "A") await this.moveUp();
                ...
                await this.goHome(true); // Shift+Home → perluas ke awal baris
              else if (seq5 === "F") await this.goEnd(true); // Shift+End → perluas ke akhir baris
              if (this.selectionActive) {
                this.selectionEnd = {
                  line: this.cursorY + this.offsetY,
                  col: this.cursorX,
            ...
            if (seq4 === "5") {
              ...
              if (seq5 === "A") {
                for (let k = 0; k < 5; k++) await this.moveUp();
                return;
              }
              if (seq5 === "B") {
                for (let k = 0; k < 5; k++) await this.moveDown();
                return;
              }
              if (seq5 === "H") {
                await this.jumpToStartOfFile();
                return;
              } // Ctrl+Home
              if (seq5 === "F") {
                await this.jumpToEndOfFile();
                return;
              } // Ctrl+End
            }
          } else if (seq3 === "~") {
        ...
        // Arrow biasa (menonaktifkan selection visual saat navigasi)
        if (seq2 === "A" || seq2 === "B" || seq2 === "C" || seq2 === "D") {
          await finalizeOnNav();
    ...
    // Ctrl+X (Cut) dengan selection aktif atau last selection yang tersimpan
    if (char === "\u0018") {
      if (this.selectionActive) {
        this.finalizeSelection();
      }
      if (this.lastSelectionStart && this.lastSelectionEnd) {
        ...
        const destLine = this.cursorY + this.offsetY;
        const destCol = this.cursorX;

        let isInside = false;
        if (destLine > s.line && destLine < e.line) {
          isInside = true;
        } else if (destLine === s.line && destLine === e.line) {
          if (destCol >= s.col && destCol <= e.col) isInside = true;
        } else if (destLine === s.line) {
          if (destCol >= s.col) isInside = true;
        } else if (destLine === e.line) {
          if (destCol <= e.col) isInside = true;
        }

        if (isInside) {
          // Cukup hapus selection saja (standard Cut)
          this.selectionActive = true;
          this.selectionStart = start;
          this.selectionEnd = end;
          await this.deleteSelection();
        } else {
          // Hitung koordinat destinasi baru setelah selection dihapus
          let newDestLine = destLine;
          let newDestCol = destCol;
          if (destLine > e.line || (destLine === e.line && destCol >= e.col)) {
            if (s.line === e.line) {
              if (destLine === s.line) {
                newDestCol = destCol - (e.col - s.col);
              }
            } else {
              if (destLine === e.line) {
                newDestLine = s.line;
                newDestCol = s.col + (destCol - e.col);
              } else {
                newDestLine = destLine - (e.line - s.line);
              }
            }
          }

          // 1. Hapus selection asal
          this.selectionActive = true;
          this.selectionStart = start;
          this.selectionEnd = end;
          await this.deleteSelection();

          // 2. Set kursor ke destinasi baru
          this.cursorX = newDestCol;
          this.monoPos = newDestCol;
          if (newDestLine < this.offsetY) {
            this.offsetY = newDestLine;
          } else if (newDestLine >= this.offsetY + this.screenHeight) {
            this.offsetY = newDestLine - this.screenHeight + 1;
          }
          this.cursorY = newDestLine - this.offsetY;

          // 3. Paste clipboard ke kursor saat ini
          const lineIdx = newDestLine;
          const line = this.lines[lineIdx] || "";
          ...
          if (parts.length === 1) {
            ...
            else if (absLine >= this.offsetY + this.screenHeight)
              this.offsetY = absLine - this.screenHeight + 1;
        ...
        // Bersihkan data selection terakhir setelah digunakan
        this.lastSelectionStart = null;
        this.lastSelectionEnd = null;
      }
      return;
    ...
    if (this.selectionActive) {
      this.finalizeSelection();
      await this.render();
      // Bersihkan data selection terakhir karena ada pengetikan
      this.lastSelectionStart = null;
      this.lastSelectionEnd = null;
    } else if (char !== "\u0018") {
      this.lastSelectionStart = null;
    ...
    if (this.findMode) {
        ...
        if (this.findStep === "find") {
          if (this.findText) {
            ...
            if (this.findResults.length > 0) {
              ...
              await this.write(
                `\x1b[${this.screenHeight + 1};1H\x1b[41m\x1b[97m No results found for "${this.findText}" \x1b[0m\x1b[K`,
              );
      ...
      if (this.findStep === "confirm") {
        ...
        if (c === "y") {
          ...
          this.findCurrentIndex =
            (this.findCurrentIndex + 1) % this.findResults.length;
          await this.jumpToResult(this.findCurrentIndex);
        } else if (c === "a") {
      ...
      if (char === "\u007f" || char === "\b") {
        if (this.findStep === "find") {
          ...
          this.replaceText = this.replaceText.slice(0, -1);
        await this.renderStatusBar();
      } else if (char >= " ") {
        if (this.findStep === "find") {
          this.findText += char;
    ...
    if (char === "\r" || char === "\n") {
      ...
      const isClose =
        afterPos >= 2 &&
        updatedLine[afterPos - 2] === "*" &&
        updatedLine[afterPos - 1] === "/";
      const isOpen =
        afterPos >= 2 &&
        updatedLine[afterPos - 2] === "/" &&
        updatedLine[afterPos - 1] === "*";
  ...
  private async moveDown() {
    if (
      ...
      this.cursorY + this.offsetY < this.lines.length - 1
    ) {
      this.cursorY++;
   ...
   * kalau tidak, pindah kursor ke awal baris.
   */
  private async goHome(expandSelection: boolean = false) {
    if (expandSelection && this.selectionActive) {
      this.cursorX = 0;
      this.offsetX = 0;
      this.monoPos = 0;
      this.selectionEnd = {
   ...
   */
  private async goEnd(expandSelection: boolean = false) {
    if (expandSelection && this.selectionActive) {
      const line = this.lines[this.cursorY + this.offsetY] || "";
      this.cursorX = line.length;
      this.monoPos = this.cursorX;
  ...
  private async pageDown() {
    if (this.offsetY + this.screenHeight < this.lines.length) {
      this.offsetY = Math.min(
        this.lines.length - this.screenHeight,
        this.offsetY + this.screenHeight,
      );
      await this.render();
    }
  }
  ...
  private async jumpToEndOfFile() {
    ...
    this.cursorY = Math.min(
      this.lines.length - this.offsetY - 1,
      this.screenHeight - 1,
    );
  ...
  private async deleteChar() {
    ...
    if (this.cursorX < line.length) {
      this.captureState();
      this.lines[lineIdx] =
        line.slice(0, this.cursorX) + line.slice(this.cursorX + 1);
      this.changed = true;
      this.checkModified();
      await this.render();
  ...
  private performFind() {
    ...
    const searchStr = isCaseSensitive
      ? this.findText
      : this.findText.toLowerCase();

    for (let i = 0; i < this.lines.length; i++) {
      const line = isCaseSensitive
        ? this.lines[i]
        : this.lines[i].toLowerCase();
  ...
  private async replaceCurrent() {
    ...
    this.lines[res.line] =
      line.slice(0, res.col) +
      this.replaceText +
      line.slice(res.col + res.length);
    this.changed = true;
    this.checkModified();
  ...
  private async replaceAll() {
    ...
    await this.render();
    await this.write(
      `\x1b[${this.screenHeight + 1};1H\x1b[42m\x1b[30m Replaced ${this.findResults.length} occurrences. \x1b[0m\x1b[K`,
    );
  ...
  private async jumpToResult(index: number) {
    ...
    if (
      targetLine < this.offsetY ||
      targetLine >= this.offsetY + this.screenHeight
    ) {
      this.offsetY = Math.max(
        0,
        targetLine - Math.floor(this.screenHeight / 2),
      );
    }
  ...
  public async askDirtyExit(): Promise<"save" | "discard" | "cancel"> {
    ...
    await this.write(
      `\x1b[${this.screenHeight + 1};1H\x1b[41m\x1b[97m${prompt}\x1b[0m\x1b[K`,
    );
    while (true) {
  ...
  private async showHelp() {
    ...
    const startRow = Math.max(
      1,
      Math.floor((this.screenHeight - helpText.length) / 2),
    );
    const startCol = Math.max(1, Math.floor((this.screenWidth - 72) / 2));
  ...
  public async save() {
    try {
      ...
      if (fd !== null) {
        ...
        await this.render();
        await this.write(
          `\x1b[${this.screenHeight + 1};1H\x1b[42m\x1b[30m ** File Saved! ** \x1b[0m\x1b[K`,
        );
        setTimeout(() => this.renderStatusBar(), 1500);
      }
    } catch (e: any) {
      await this.write(
        `\x1b[${this.screenHeight + 1};1H\x1b[41m\x1b[37m Error: ${e.message} \x1b[0m\x1b[K`,
      );
    }
  }
