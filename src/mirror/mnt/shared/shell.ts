...
export class main implements IProgram {
    ...
    private async renderPrompt(): Promise<string> {
        ...
        const userColor = "\u001b[96m";   // Bright Cyan
        const hostColor = "\u001b[92m";   // Bright Green
        const dirColor = "\u001b[93m";    // Bright Yellow
        const accentColor = "\u001b[97m"; // Bright White
        const typeColor = isRoot ? "\u001b[91m" : "\u001b[97m"; // Bright Red for root #, White for others
        const reset = "\u001b[0m";
    ...
    private async readLine(prompt: string): Promise<string> {
        ...
        while (true) {
            ...
            if (compState && compState.active) {
                if (char === "\t") {
                    ...
                    if (next1 === "[") {
                        ...
                        if (next2 === "A") { // Up
                            compState.selectedIdx = (compState.selectedIdx - 1 + compState.matches.length) % compState.matches.length;
                            await this.renderCandidatesInPlace(compState, this.lineBuffer.length - this.cursor);
                            continue;
                            ...
                            if (compState.selectedIdx < 0) {
                                compState.selectedIdx = (compState.selectedIdx + Math.ceil(matches.length / numRows) * numRows) % matches.length;
                            }
                            await this.renderCandidatesInPlace(compState, this.lineBuffer.length - this.cursor);
                            continue;
                        }
            ...
            if (char === "\r" || char === "\n") { // Enter
            ...
            } else if (char === "\u001b") { // ESC Sequence
                const next1 = await this.std.getChar();

                if (next1 === "[") {
                    const next2 = await this.std.getChar();
                    if (next2 === "A") { // Up (History)
                        if (historyPos > 0) {
                            historyPos--;
                        ...
                        this.cursor = this.lineBuffer.length;
                        await redrawLine();
                    } else if (next2 === "1") { // Home (some terminals)
                        if (await this.std.getChar() === "~") {
                            this.cursor = 0;
                            await redrawLine();
                        }
                    } else if (next2 === "4") { // End (some terminals)
                        if (await this.std.getChar() === "~") {
                            this.cursor = this.lineBuffer.length;
                            await redrawLine();
                        }
                    } else if (next2 === "3") { // Delete
                        if (await this.std.getChar() === "~") {
                            if (this.cursor < this.lineBuffer.length) {
                                this.lineBuffer = this.lineBuffer.slice(0, this.cursor) + this.lineBuffer.slice(this.cursor + 1);
                                await redrawLine();
                            }
                        }
                    }
                } else if (next1 === "O") { // ESC O sequence (some terminals)
                    const next2 = await this.std.getChar();
                    if (next2 === "H") { // Home
                        this.cursor = 0;
                        await redrawLine();
            ...
            } else if (char === "\t") { // TAB (Trigger Completion)
                ...
                if (result.matches.length === 1) {
                    ...
                    const fileNamePart = lastPart.includes("/") ? lastPart.substring(lastPart.lastIndexOf("/") + 1) : lastPart;
            ...
            } else if (char && char >= " ") { // Insert at cursor
    ...
    private async getCompletionMatches(partial: string): Promise<{ matches: string[], commonBase: string, originalParts: string[], dirPath: string }> {
        ...
        if (lastSlash !== -1) {
            searchDir = lastWord.substring(0, lastSlash + 1);
            filePrefix = lastWord.substring(lastSlash + 1);
        }
        let candidates: string[] = [];
        const isCD = (cmd === "cd");
        if (parts.length === 1 && !isCD && lastSlash === -1) {
            const builtIns = ["cd", "exit", "help", "version", "export", "echo", "history"];
            builtIns.forEach(b => {
                if (b.startsWith(lastWord)) candidates.push(b + " ");
            });
        ...
        // If the command is 'sudo', we want to complete the SECOND part as a command.
        if (cmd === "sudo" && parts.length === 2 && lastSlash === -1) {
            const builtIns = ["cd", "exit", "help", "version", "export", "echo", "history"];
            builtIns.forEach(b => {
                if (b.startsWith(lastWord)) candidates.push(b + " ");
            });
        ...
        candidates.sort();
        return { matches: candidates, commonBase: searchDir, originalParts: parts, dirPath: searchDir };
    }
    ...
    private async handleCommand(input: string): Promise<string> {
        ...
        if (trimmedInput.includes("|")) {
            result = await this.executePipeline(trimmedInput);
        } else {
            const execResult = await this.executeSingleCommand(trimmedInput);
    ...
    private async executeSingleCommand(input: string, externalStdinFd?: number, externalStdoutFd?: number, forceBackground?: boolean, isPipelinePart: boolean = false): Promise<string | { pid: number, name: string }> {
        let trimmedInput = input.trim();
        if (!trimmedInput) return "";
        ...
        const rawArgs = parts.slice(1).map(arg => {
            const isQuoted = arg.startsWith('"') && arg.endsWith('"');
            return {
                text: isQuoted ? arg.substring(1, arg.length - 1) : arg,
                quoted: isQuoted
            };
        });
        ...
        // 4. Handle Built-ins
        if (cmd === "help") {
            result = "Available commands: cd, exit, export, echo, version, help, history, ps, whoami";
        } else if (cmd === "echo") {
            result = args.join(" ");
        } else if (cmd === "cd") {
            const target = args[0] || "/";
            const success = await this.shell.chdir(target);
            if (!success) result = `-bash: cd: ${target}: No such file or directory`;
        } else if (cmd === "version") {
    ...
    private async expandArguments(rawArgs: { text: string, quoted: boolean }[]): Promise<string[]> {
        let expanded: string[] = [];
        const home = await this.shell.getenv("HOME") || "/root";

        for (const arg of rawArgs) {
            let processed = await this.expandVariables(arg.text);
            ...
            else if (processed.startsWith("~/")) processed = home + processed.substring(1);
            ...
            const matches = files
                ...
                .map((f: any) => {
                    if (isAbsolute) {
                        ...
                        return relativePrefix ? (relativePrefix.endsWith("/") ? relativePrefix + f.name : relativePrefix + "/" + f.name) : f.name;
                    }
                });
