...
import { formatNetFSSpec, parseNetFSSpec } from "../common/netfs/NetFSProtocol";
import { interpreterCandidates, isShellInterpreter, parseShebang } from "../common/Shebang";
...
export class SyscallDispatcher {
    ...
    private pendingDbRequests: Map<string, { resolve: (v: any) => void; reject: (e: Error) => void }> = new Map();
    ...
    private netSniffers: Map<string, Map<number, { root: boolean; decrypt: boolean }>> = new Map();
    // Driver yang sudah di-wire onSniff (hindari duplikat callback)
    private wiredSniffers = new Set<SimpleMQTNLDriver>();

    /**
     * Syscall yang tidak pernah dilog DEBUG (frekuensi tinggi / polling).
     * Set (bukan array lokal) → cek O(1) tanpa alokasi per request.
     */
    private static readonly SILENT_SYSCALLS = new Set<SyscallCode>([
        SyscallCode.READ,
        SyscallCode.PS,
        SyscallCode.GETCWD,
        SyscallCode.WHOAMI,
     ...
     */
    private async forwardDbRequest(pid: number, op: string, args: any): Promise<any> {
    ...
    public async handleRequest(req: SyscallRequest): Promise<any> {
        // Log DEBUG dibangun LAZY: template string + reverse-enum lookup dilewati
        // total saat level bukan DEBUG. Untuk syscall frekuensi tinggi (LCD 30 fps,
        // polling) ini menghapus alokasi string per request.
        if (Logger.currentLevel === LogLevel.DEBUG && !SyscallDispatcher.SILENT_SYSCALLS.has(req.code)) {
            this.logger.debug(`[IPC] PID ${req.pid} Request: ${SyscallCode[req.code]} (${req.requestId})`);
        }
        ...
        try {
            ...
            this.logger.error(`Syscall Error [${SyscallCode[req.code]}]: ${e.message}`);
            throw e;
        }
    }
    ...
    private validateArgs(code: SyscallCode, args: any) {
        ...
        switch (code) {
            case SyscallCode.WRITE:
                if (typeof args === "object" && args.pid !== undefined) {
                    if (args.content === undefined)
                        throw new Error("Invalid WRITE arguments: content missing for PID-based route");
            ...
            case SyscallCode.READ:
                if (typeof args !== "number" && (typeof args !== "object" || args.pid === undefined)) {
                    throw new Error(`Syscall ${SyscallCode[code]} expects a numeric argument or bridge object`);
                }
                break;
            ...
            case SyscallCode.MMAP:
                if (typeof args !== "number")
                    throw new Error(`Syscall ${SyscallCode[code]} expects a numeric argument`);
                break;
            case SyscallCode.OPEN:
                if (typeof args !== "string" && (typeof args !== "object" || !args.path))
            ...
            case SyscallCode.SENDTO:
                if (typeof args.fd !== "number" || !args.address || typeof args.port !== "number")
                    throw new Error("Invalid SENDTO arguments");
                break;
            case SyscallCode.EXEC:
            case SyscallCode.REEXEC:
                if (!args.path || !Array.isArray(args.args)) throw new Error(`Invalid ${SyscallCode[code]} arguments`);
                break;
            case SyscallCode.READ_CHUNK:
                if (!args.path || typeof args.offset !== "number" || typeof args.length !== "number") {
                    throw new Error("READ_CHUNK requires { path, offset, length }");
                }
                break;
            case SyscallCode.WRITE_CHUNK:
                if (!args.path || args.chunk === undefined || typeof args.offset !== "number") {
    ...
    public async dispatch(pid: number, code: SyscallCode, args: any): Promise<any> {
        const pcb = this.scheduler.getProcess(pid);
        if (!pcb) {
        ...
        switch (code) {
            case SyscallCode.MKDIR: {
                ...
                const { vfs, relativePath } = this.mountManager.resolve(absoluteMkdirPath);

                // Permission Check
                const parentPath = absoluteMkdirPath.substring(0, absoluteMkdirPath.lastIndexOf("/")) || "/";
                const { vfs: parentVfs, relativePath: parentRelativePath } = this.mountManager.resolve(parentPath);
                const parentNode = await parentVfs.stat(parentRelativePath);

                if (parentNode && !this.satpam.check(pcb, parentNode, Permission.WRITE)) {
                    throw new Error("Permission Denied: Cannot write to parent directory.");
                }
            ...
            case SyscallCode.LS: {
                ...
                if (!lsNode) {
                    throw new Error(`ls: cannot access '${absoluteLsPath}': No such file or directory`);
                }
                if (!this.satpam.check(pcb, lsNode, Permission.READ)) {
                    throw new Error(`ls: cannot open directory '${absoluteLsPath}': Permission denied`);
                }
            ...
            case SyscallCode.OPEN: {
                ...
                // 0. Resolve VFS
                const { vfs, relativePath } = this.mountManager.resolve(absoluteOpenPath);

                // 1. Permission Check
                const requiredPerm = flags.includes("w") || flags.includes("+") ? Permission.WRITE : Permission.READ;
                ...
                if (node) {
                    ...
                    if (requiredPerm === Permission.WRITE && !absoluteOpenPath.startsWith("/dev/")) {
                        const parentDir = absoluteOpenPath.substring(0, absoluteOpenPath.lastIndexOf("/")) || "/";
                        ...
                        if (parentNode) {
                            if (!this.satpam.check(pcb, parentNode, Permission.WRITE)) {
                                throw new Error(`Permission Denied: Cannot create file in ${parentDir}`);
                            }
                        }
                    }
                    if (requiredPerm === Permission.WRITE && !node && !absoluteOpenPath.startsWith("/dev/")) {
                        await vfs.touch(relativePath, "", pcb.uid, pcb.gid, 420); // 644 equivalent
                    }

                    // Truncate if Opening with 'w' (Write) and NOT 'a' (Append)
                    if (node && flags.includes("w") && !flags.includes("a") && !absoluteOpenPath.startsWith("/dev/")) {
                ...
                if (absoluteOpenPath.startsWith("/dev/")) {
                    ...
                    if (!device) {
                        if (devName === "tty") {
                            ...
                            if (!isNaN(ptyId)) {
                                device = (this.kernel.getPTYManager?.()?.getSlave(ptyId) as unknown as IDevice) || null;
                            }
                    ...
                    if (device) {
                        ...
                        const isStdDev = stdoutAliases.includes(devName) || stdinAliases.includes(devName);
                        if (!isStdDev) {
                            ...
                            if (!this.satpam.check(pcb, devPerm, requiredPerm)) {
                                throw new Error(`Permission Denied: You cannot access device /dev/${devName}`);
                            }
                        }
                        ...
                        if ((f.includes("w") || f.includes("a")) && device.ioctl) await device.ioctl(20, null); // INC_WRITE_REF
                ...
                if (flags && flags.includes("w") && !flags.includes("a") && !flags.includes("+")) {
            ...
            case SyscallCode.STAT: {
                ...
                const { vfs, relativePath } = this.mountManager.resolve(absoluteStatPath);
            ...
            case SyscallCode.UNLINK: {
                ...
                if (!this.satpam.check(pcb, node, Permission.WRITE)) throw new Error("Permission Denied");
            ...
            case SyscallCode.MMAP: {
                // Memory-map buffer device (mis. framebuffer LCD) untuk zero-copy:
                // device mengembalikan SharedArrayBuffer yang dipakai bersama worker.
                // Device yang belum mendukung → null (userland harus fallback).
                const fd = args as number;
                const entry = pcb.fdTable[fd];
                if (!entry) throw new Error(`FD NOT FOUND: ${fd}`);
                if (entry.device.mmap) return entry.device.mmap();
                return null;
            }
                ...
                if (entry) {
                    ...
                    if (device instanceof SocketDevice) {
                        ...
                        if (socket.bound && socket.driver) {
                            ...
                            if (port !== null) {
                                this.logger.debug(`Closing socket on port ${port}, releasing resources.`);
                    ...
                    if ((f.includes("w") || f.includes("a")) && device.ioctl) await device.ioctl(21, null); // DEC_WRITE_REF
            ...
            case SyscallCode.PS: {
                ...
                const mem = includeMemory
                    ? await Promise.all(processes.map((p) => this.scheduler.getProcessMemory(p.pid)))
                    : [];
            ...
            case SyscallCode.KILL: {
                ...
                const targetPcb = this.scheduler.getProcess(targetPid);
                if (!targetPcb) throw new Error(`kill: No such process (PID ${targetPid})`);
            ...
            case SyscallCode.IOCTL: {
                ...
                if (cmd === 2 && entry.device instanceof TTYDevice) {
                    ...
                    let targetTtyId = arg !== null && arg !== undefined ? parseInt(arg) : NaN;
            ...
            case SyscallCode.EXEC: {
                ...
                if (!node) {
                    ...
                    for (const ext of extensions) {
                        ...
                        const { vfs: altVfs, relativePath: altRelativePath } = this.mountManager.resolve(altPath);
                ...
                if (!node) {
                    this.logger.error(`EXEC Failed: File not found [${absoluteExecPath}]`);
                ...
                if (appContent && !/\.(ts|js)$/i.test(absoluteExecPath)) {
                    ...
                    if (shebang) {
                        ...
                        if (!this.satpam.check(pcb, interpreter.node, Permission.EXECUTE)) {
                            throw new Error(`Permission Denied: Cannot execute ${interpreter.path}`);
                        }

                        this.logger.info(`[EXEC] shebang: ${absoluteExecPath} → ${interpreter.path}`);
                ...
                this.logger.debug(`[EXEC] Using VFS content for ${absoluteExecPath} (Direct Memory)`);

                let stdoutDevice = pcb.fdTable[1]?.device || this.kernel.devices!.stdout;
                let stdinDevice = pcb.fdTable[0]?.device || this.kernel.devices!.stdin;
                let stderrDevice = pcb.fdTable[2]?.device || this.kernel.devices!.stderr;
                ...
                if (node && node.mode & 2048) {
                    ...
                    this.logger.info(`SetUID Execution detected for ${absoluteExecPath}: Running as UID ${targetUid}`);
                }
                ...
                const newPcb = this.scheduler.createProcess(binaryName, {
                    ...
                    ttyId: ptyId !== undefined && ptyId >= 0 ? -(ptyId + 1) : ttyId !== undefined ? ttyId : pcb.ttyId,
                    ...
                    try {
                        ...
                        if (content) {
                            const lines = content
                                ...
                                .map((l) => l.trim())
                                .filter((l) => l.length > 0);
                            const entry = lines.find((l) => parseInt(l.split(":")[2]) === newUid);
                            if (entry) {
                                pcb.owner = entry.split(":")[0];
                            }
                        }
                    } catch (e: any) {
                        this.logger.warn(`Failed to resolve username for UID ${newUid}: ${e.message}`);
                    }
                }
            ...
            case SyscallCode.SETGID: {
                ...
                if (!this.isRoot(pcb))
                    throw new Error("Permission Denied: Only root or root group members can change GID");
                pcb.gid = newGid;
                return true;
            }
            ...
            case SyscallCode.SETGROUPS: {
                ...
                if (!this.isRoot(pcb))
                    throw new Error("Permission Denied: Only root or root group members can change groups");
            ...
            case SyscallCode.SEND_MSG: {
                ...
                if (typeof targetPid === "string") {
                    ...
                    if (resolvedId === undefined) throw new Error(`Destination Identity "${targetPid}" not found.`);
            ...
            case SyscallCode.GUI_REQ: {
                ...
                if (!payload || payload.syscall !== "GUI_REQ") {
                    this.logger.error(`PID ${pid} sent malformed GUI_REQ payload. SIGKILL.`);
                    await this.scheduler.kill(pid, 9);
                    throw new Error("GUI_REQ: Invalid payload format — syscall must be 'GUI_REQ'");
                }
                if (!payload.action || !Object.values(GUIAction).includes(payload.action)) {
                ...
                if (payload.action === GUIAction.REGISTER_DAEMON) {
                    ...
                    this.logger.info(`PixelSpace: Process ${pid} (${pcb.name}) registered as DOME engine`);
                ...
                if (payload.action === GUIAction.CREATE_WINDOW) {
                    ...
                    this.logger.info(`PixelSpace: PID ${pid} created window '${payload.wid}' ("${title}")`);
                } else {
                    const ownerPid = guiRegistry.getOwner(payload.wid);
                    const isOwner = ownerPid !== null && ownerPid === pid;
                    const isParentManagedAction =
                        (payload.action === GUIAction.MINIMIZE_WINDOW || payload.action === GUIAction.RESTORE_WINDOW) &&
                        ownerPid !== null &&
                    ...
                    if (!isAuthorized) {
                        ...
                        throw new Error(`GUI_REQ: Access Denied — PID ${pid} does not own window '${payload.wid}'`);
                    }
                    ...
                    if (payload.action === GUIAction.DESTROY_WINDOW) {
                        ...
                        this.logger.info(`PixelSpace: Window '${payload.wid}' destroyed by PID ${pid}`);
                    }
                ...
                const forwarded = this.scheduler.sendEvent(guedPid, "gui_request", payload);
                if (!forwarded) {
                    // gued mungkin mati — rollback CREATE jika perlu
                    if (payload.action === GUIAction.CREATE_WINDOW) {
                        guiRegistry.destroyWindow(payload.wid);
                    }
                    throw new Error("GUI_REQ: Failed to forward to DOME engine (process may be dead)");
                }
            ...
            case SyscallCode.BIND: {
                ...
                const entry = pcb.fdTable[fd];
                if (!entry || !(entry.device instanceof SocketDevice)) throw new Error("Invalid Socket FD");
                ...
                if (address) {
                    ...
                    for (const key in this.kernel.devices) {
                        ...
                        if (dev instanceof SimpleMQTNLDriver) {
                            if ((dev as any).localAddress === address || dev.name === address) {
                                targetDriver = dev;
                                break;
                            }
                        }
                    ...
                    // Use Default
                    targetDriver = this.kernel.devices[cfg.network.defaultDevice] as SimpleMQTNLDriver;
                }

                if (!targetDriver) throw new Error(`Network Interface not found for address: ${address || "DEFAULT"}`);
                ...
                if (port === 0) {
                    ...
                    if (!portMgr.allocatePort(port, pcb.pid)) throw new Error(`Port ${port} already in use`);
                }
            ...
            case SyscallCode.SENDTO: {
                ...
                if (!mqtnl) {
                    const cfg = Config.get();
                    mqtnl = this.kernel.devices[cfg.network.defaultDevice] as SimpleMQTNLDriver;
                }

                if (!mqtnl) throw new Error("No network interface available");

                const finalSrcPort = argLocalPort || socket.getPort() || 0;
                return await mqtnl.send(address, port, data, flag ?? PacketFlags.FLAG_DATA, finalSrcPort);
            }
            ...
            case SyscallCode.SET_NET_DEFAULT: {
                ...
                this.kernel.bootLog?.(`NET: default interface ${previous} -> ${target.name} by PID ${pid}`);
            ...
            case SyscallCode.SHUTDOWN: {
                if (!this.isRoot(pcb))
                    throw new Error("Permission Denied: Only root or root group members can shutdown original system");
                ...
                this.kernel.bootLog(`Graceful Wait: Monitoring process termination (max 5s)...`);
                ...
                this.kernel.bootLog(`Halting: Terminating PID 1 with exit code ${exitCode}.`);
                this.kernel.bootLog(exitCode === 1 ? "System is rebooting." : "System is halting.");
            ...
            case SyscallCode.REEXEC: {
                ...
                this.logger.info(`[REEXEC] Process ${pid} is replacing itself with ${appPath}`);
            ...
            case SyscallCode.REPARENT: {
                ...
                if (!childPcb) throw new Error(`REPARENT: No such process ${targetPid}`);
            ...
            case SyscallCode.SYNC_FROM_HOST: {
                if (!this.isRoot(pcb))
                    throw new Error("Permission Denied: Only root or root group members can perform sync from host.");
                const { vfsPath, hostPath } = args;
                ...
                if (cleanVfsPath.startsWith("/bin/") || cleanVfsPath.startsWith("/root/")) {
                    mode = 493; // 0755
                }

                // Driver uses touch for writing/updating file content
                await vfs.touch(relativePath, content, pcb.uid, pcb.gid, mode);
                this.logger.info(`[SYNC] Pulled HOST:${hostPath} -> VFS:${cleanVfsPath} (Mode: ${mode.toString(8)})`);
                return true;
            }

            case SyscallCode.MOUNT: {
                if (!this.isRoot(pcb)) throw new Error("Permission Denied: Only root or root group members can mount.");
                ...
                const { vfs: mntVfs, relativePath: mntRel } = this.mountManager.resolve(fullVfsPath);
                ...
                if (fsType === "bkfs") {
                    ...
                    driver = new BKFS(path.resolve(process.cwd(), hostPath), readOnly || false);
                ...
                this.mountManager.mount(fullVfsPath, driver, fsType, hostPath, readOnly || false, uid, gid);
                return true;
            }

            case SyscallCode.UMOUNT: {
                if (!this.isRoot(pcb))
                    throw new Error("Permission Denied: Only root or root group members can unmount.");
            ...
            case SyscallCode.GET_USAGE: {
                ...
                const m = this.mountManager.listMounts().find((mt) => mt.vfsPath === mountPoint);
                if (m && m.type === "bkfs") {
            ...
            case SyscallCode.READ_CHUNK: {
                const { path: chunkPath, offset, length } = args as { path: string; offset: number; length: number };
            ...
            case SyscallCode.WRITE_CHUNK: {
                const { path: chunkPath, chunk, offset } = args as { path: string; chunk: string; offset: number };
                const absolutePath = PathResolver.resolve(pcb.cwd, chunkPath);
                ...
                if (node) {
                    ...
                    const { vfs: parentVfs, relativePath: parentRel } = this.mountManager.resolve(parentDir);
            ...
            case SyscallCode.SET_IDENTITY: {
                ...
                if (!success) {
                    this.logger.warn(`SET_IDENTITY: UUID ${uuid} already held by another active process`);
                }
                return success;
            }
            ...
            case SyscallCode.NET_SNIFFER_REGISTER: {
                ...
                this.netSniffers.get(iface)!.set(pid, { root: isRootSniffer, decrypt: wantDecrypt });
                this.logger.info(
            ...
            case SyscallCode.PTY_ALLOC: {
                ...
                pair.slave.onInterrupt = () => {
                    ...
                    if (fgPid) {
                        this.logger.info(`[PTY] Sending SIGINT to PID ${fgPid} (PTY${pair.id} Ctrl+C)`);
                        this.scheduler?.sendEvent(fgPid, "signal", "SIGINT");
                    }
                };

                this.logger.info(`[PTY] PID ${pid} allocated PTY${pair.id} (/dev/pts/${pair.id})`);
                return {
                    id: pair.id,
