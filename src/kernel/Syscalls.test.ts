...
describe("SyscallDispatcher (A1)", () => {
  ...
  describe("READ", () => {
    it("A1.08 READ – read entire file content", async () => {
      ...
      const fd = await dispatcher.dispatch(
        pcb.pid,
        SyscallCode.OPEN,
        "/home/test.txt",
      );
  ...
  describe("CLOSE", () => {
    ...
    it("A1.22 CLOSE – all fds auto-closed on process exit", async () => {
      ...
      const fd1 = await dispatcher.dispatch(
        pcb.pid,
        SyscallCode.OPEN,
        "/home/test.txt",
      );
  ...
  describe("Process Identity", () => {
    ...
    it("A1.96 SETUID – requires root", async () => {
      const userPcb = createTestProcess({
        uid: 1000,
        gid: 1000,
        owner: "user",
        groups: [1000],
  ...
  describe("WAITPID", () => {
    it("A1.36 WAITPID – wait for specific child", async () => {
      ...
      const exitCode = await dispatcher.dispatch(
        parent.pid,
        SyscallCode.WAITPID,
        20,
      );
  ...
  // ============================================================
  // SET_NET_DEFAULT – ubah interface MQTNL default saat runtime
  // ============================================================
  describe("SET_NET_DEFAULT", () => {
    it("non-root ditolak", async () => {
      const pcb = createTestProcess({
        uid: 1000,
        gid: 1000,
        owner: "user",
      ...
      await expect(
        dispatcher.dispatch(pcb.pid, SyscallCode.SET_NET_DEFAULT, {
          deviceName: "smqtnl1",
        }),
      ).rejects.toThrow("Permission Denied");
    });

    it("interface tidak dikenal ditolak", async () => {
      const pcb = createTestProcess();
      await expect(
        dispatcher.dispatch(pcb.pid, SyscallCode.SET_NET_DEFAULT, {
          deviceName: "tidak-ada",
        }),
      ).rejects.toThrow("Network Interface not found");
    });

    it("root bisa ganti default via deviceName/address (in-memory saja)", async () => {
      const pcb = createTestProcess();
      const cfg = Config.get();
      const previous = cfg.network.defaultDevice;

      // Driver tiruan: cukup lewat prototype agar `instanceof` cocok,
      // TANPA menjalankan constructor (yang membuat koneksi MQTT + timer).
      const dev0: any = Object.create(SimpleMQTNLDriver.prototype);
      dev0.name = "smqtnl0";
      dev0.localAddress = "mactsix";
      const dev1: any = Object.create(SimpleMQTNLDriver.prototype);
      dev1.name = "smqtnl1";
      dev1.localAddress = "mactsix_2";
      kernel.devices.smqtnl0 = dev0;
      kernel.devices.smqtnl1 = dev1;

      try {
        const byName = await dispatcher.dispatch(
          pcb.pid,
        ...
        expect(byName.defaultDevice).toBe("smqtnl1");
        expect(Config.get().network.defaultDevice).toBe("smqtnl1");

        // Bisa juga lewat address → dinormalisasi ke deviceName kanonik.
        const byAddr = await dispatcher.dispatch(
          pcb.pid,
          SyscallCode.SET_NET_DEFAULT,
          {
            deviceName: "mactsix",
        ...
        expect(byAddr.defaultDevice).toBe("smqtnl0");
        expect(Config.get().network.defaultDevice).toBe("smqtnl0");
      } finally {
        cfg.network.defaultDevice = previous;
        delete kernel.devices.smqtnl0;
        delete kernel.devices.smqtnl1;
      }
    });
  });
