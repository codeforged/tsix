...
export class SimpleMQTNLDriver implements IDevice {
  ...
  // [SECURITY AGENT REGISTRY] Pabrik agent enkripsi per-nama (Jalur A).
  // Driver TIDAK lagi meng-hardcode `new SecurityAgent()` — agent dipilih
  // via nama string dari ioctl UPGRADE_SECURITY. Default "chacha20".
  // Agent kustom didaftarkan lewat registerAgent() (SOP jenis enkripsi baru).
  private static agentRegistry: Map<string, () => ISecurityAgent> = new Map<
    string,
    () => ISecurityAgent
  >([
    ["chacha20", () => new SecurityAgent()],
    ["aes-gcm", () => new AesGcmAgent()],
  ]);

  /** registerAgent(): Daftarkan factory agent kustom (SOP). */
  public static registerAgent(
    name: string,
    factory: () => ISecurityAgent,
  ): void {
    SimpleMQTNLDriver.agentRegistry.set(name, factory);
  }

  /** getAgent(): Instantiate agent berdasarkan nama (default "chacha20"). */
  public static getAgent(name?: string): ISecurityAgent {
    if (name && SimpleMQTNLDriver.agentRegistry.has(name)) {
      return SimpleMQTNLDriver.agentRegistry.get(name)!();
    }
    return new SecurityAgent();
  }

  /** listAgents(): Nama semua agent enkripsi yang terdaftar di registry. */
  public static listAgents(): string[] {
    return [...SimpleMQTNLDriver.agentRegistry.keys()];
  }
  ...
  private receivedPackets: Map<
    string,
    { total: number; received: Map<number, Buffer | string>; timestamp: number }
  > = new Map();
  ...
  private handleIncomingMessage(topic: string, message: Buffer) {
    try {
      ...
      const magicByte = Buffer.isBuffer(message)
        ? message[0]
        : (message as string).charCodeAt(0);
      const proto = this.protocols.find((p) =>
        p.getMagicChars().includes(magicByte),
      );
      if (!proto) {
        this.logger.error(
          `Protocol mismatch on ${topic}. Magic: 0x${message[0].toString(16)}`,
        );
        return;
      }
      ...
      if (
        packet.header.srcAddress === localAddr &&
        packet.header.dstAddress !== localAddr
      ) {
        return;
      }
      ...
      if (
        ...
        this.logger.debug(
          `[FILTER DROP] Packet for ${packet.header.dstAddress} dropped (Local: ${localAddr})`,
        );
        return;
      }
      ...
      if (session.received.size < session.total) {
        ...
        if (session.total > 1) {
          this.logger.debug(
            `[REASSEMBLY] Progress ${key}: ${session.received.size}/${session.total}`,
          );
        }
        return;
      }
      ...
      const entries = Array.from(session.received.entries()).sort(
        (a, b) => a[0] - b[0],
      );
      ...
      if (session.total > 1) {
        this.logger.debug(
          `[REASSEMBLY] Complete ${key}. Total Size: ${assembledPayload.length}`,
        );
      }
      ...
      if (!decryptedPayload && assembledPayload.length > 0) {
        this.logger.warn(
          `[SECURITY] Decryption failed for ${Buffer.isBuffer(assembledPayload) ? "binary" : "legacy"} packet from ${packet.header.srcAddress}:${packet.header.srcPort} to port ${dstPort}.`,
        );
        return;
      }
      ...
      this.emitSniff({
        ...
        data: Buffer.isBuffer(decryptedPayload)
          ? decryptedPayload.toString("utf8")
          : decryptedPayload,
        raw: Buffer.isBuffer(assembledPayload)
          ? assembledPayload.toString("utf8")
          : assembledPayload,
      ...
      if (flag === PacketFlags.FLAG_PING_REQUEST && dstPort === 65535) {
        this.logger.info(
          `Responded to PING REQUEST from ${packet.header.srcAddress}`,
        );
      ...
      if (flag === PacketFlags.FLAG_BROADCAST_PING && dstPort === 65534) {
        this.logger.info(
          `Responded to BROADCAST PING from ${packet.header.srcAddress}`,
        );
      ...
      if (handler) {
        ...
        this.logger.info(
          `[DISPATCH] Port ${packet.header.dstPort}, BinaryFlag: ${isBinary}`,
        );
  ...
  private cleanupExpiredPackets() {
    ...
    for (const [key, session] of this.receivedPackets.entries()) {
      if (now - session.timestamp > ttl) {
        this.logger.warn(
          `[REASSEMBLY] Dropped incomplete packet from ${key} (TTL Expired)`,
        );
        this.receivedPackets.delete(key);
      }
    }
  ...
  public async send(
    ...
    if (packetCount > 1) {
      this.logger.debug(
        `[FRAGMENTATION] Sending ${securedPayload.length} bytes in ${packetCount} chunks...`,
      );
    }
    ...
    for (let i = 0; i < packetCount; i++) {
      ...
      if (localTarget) {
        ...
        this.logger.info(
          `[DRIVER] MQTNL LOOPBACK -> ${address} (${payloadBuffer.length} bytes)`,
        );
        ...
        await new Promise((resolve) => {
          this.logger.info(
            `[DRIVER] MQTNL Publish to ${topic} (${payloadBuffer.length} bytes)`,
          );
          this.client!.publish(
            topic,
            payloadBuffer,
  ...
  public ioctl(cmd: number, arg: any): any {
    if (cmd === 0x1001) {
      // SMQTNL_IOCTL_UPGRADE_SECURITY
      const { port, sessionKey, agent } = arg as {
        port?: number;
        sessionKey?: any;
        agent?: string;
      };
      if (port && sessionKey) {
        // Agent dipilih via nama (default "chacha20") — bukan hardcode.
        const agentInstance = SimpleMQTNLDriver.getAgent(agent);
        agentInstance.setSessionKey(sessionKey);
        this.portSecurity.set(port, agentInstance);
        this.logger.info(
          `✅ SECURITY UPGRADED for port ${port}. Agent=${agent || "chacha20"} is now ACTIVE.`,
        );
        return true;
      }
    }
    if (cmd === 0x1002) {
      ...
      this.logger.info(
        `Interface ${this.name} protocol switched to: ${this.activeProtocol.getName()}`,
      );
      return true;
    }
    return true;
