...
export interface LM6029NativeHandle {
    ...
    drawBitmap(x: number, y: number, bitmap: Buffer, w: number, h: number, color: number): void;
...
export class LM6029Device implements IDevice {
    ...
    public open(): boolean {
        ...
        try {
            ...
            if (this.initialRotation !== undefined) lcd.setRotation(num(this.initialRotation));
            if (this.initialContrast !== undefined)
                lcd.setContrast(Math.max(0, Math.min(63, num(this.initialContrast))));
            if (this.initialBacklight !== undefined) lcd.setBacklight(!!this.initialBacklight);
            if (this.initialInvert !== undefined) lcd.setDisplayInvert(!!this.initialInvert);
            if (this.initialDisplayOn !== undefined) lcd.setDisplayOn(!!this.initialDisplayOn);
    ...
    public write(data: any): boolean {
        ...
        try {
            ...
            if (data && typeof data === "object" && typeof data.op === "string") {
                ...
                if (this.autoFlush && data.op !== "display" && data.op !== "flush") this.flush();
                return true;
            }
        ...
        try {
            switch (cmd) {
                ...
                case LCDIOCTL.DRAW_LINE: {
                    ...
                    lcd.drawLine(num(x0), num(y0), num(x1), num(y1), color === undefined ? 1 : num(color));
                    return true;
                }
                ...
                case LCDIOCTL.FILL_RECT: {
                    ...
                    fn.call(lcd, num(x), num(y), num(w), num(h), color === undefined ? 1 : num(color));
                    return true;
                }
                case LCDIOCTL.DRAW_CIRCLE:
                case LCDIOCTL.FILL_CIRCLE: {
                    const [x, y, r, color] = positional(arg, ["x", "y", "r", "color"]);
                    const fn = cmd === LCDIOCTL.DRAW_CIRCLE ? lcd.drawCircle : lcd.fillCircle;
                ...
                case LCDIOCTL.FILL_TRIANGLE: {
                    ...
                    const fn = cmd === LCDIOCTL.DRAW_TRIANGLE ? lcd.drawTriangle : lcd.fillTriangle;
                    fn.call(
                        lcd,
                        num(x0),
                        num(y0),
                ...
                case LCDIOCTL.FILL_ROUND_RECT: {
                    ...
                    const fn = cmd === LCDIOCTL.DRAW_ROUND_RECT ? lcd.drawRoundRect : lcd.fillRoundRect;
                    fn.call(lcd, num(x), num(y), num(w), num(h), num(r), color === undefined ? 1 : num(color));
                    return true;
                }
                ...
                case LCDIOCTL.PRINT_TEXT: {
                    ...
                    lcd.printText(String(text ?? ""), num(x), num(y), size === undefined ? 1 : num(size));
                    if (this.autoFlush) this.flush();
                    return true;
                }
    ...
    public getInfo(): Record<string, any> {
        ...
        return {
            ...
            spiDevice: alive ? this.safe(() => this.lcd!.getSpiDevicePath?.() ?? null) : null,
    ...
    private loadAddonModule(): LM6029NativeModule | null {
        ...
        if (process.env.TSIX_LCD_ADDON_PATH) candidates.push(process.env.TSIX_LCD_ADDON_PATH);
        ...
        candidates.push(path.resolve(__dirname, "..", "..", "..", "..", "..", "lm6029acw"));
        candidates.push(path.resolve(__dirname, "..", "..", "..", "..", "..", "raspi-lcd-addon"));
