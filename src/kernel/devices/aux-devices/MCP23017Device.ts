...
export class MCP23017Device implements IDevice {
    ...
    public init(ctx: KContext): void {
        ...
        if (this.i2cBusNumber !== undefined) {
            ctx.syslog(`Inisialisasi Hardware MCP23017 pada 0x${this.address.toString(16)} (Bus ${this.i2cBusNumber})`);
        }
    ...
    public pinMode(pin: number, pinMode: number): boolean {
        if (!this.initialized) {
            console.error(`[MCP23017] Device ${this.name} not initialized. Cannot set pin mode.`);
            return false;
        }
    ...
    public digitalWrite(pin: number, value: number): boolean {
        if (!this.initialized) {
            console.error(`[MCP23017] Device ${this.name} not initialized. Cannot write to pin.`);
            return false;
        }
        if (pin < 0 || pin > 15) return false;
        ...
        try {
            ...
            console.error(`[MCP23017] digitalWrite error on pin ${pin}: ${err.message}`);
            return false;
        }
    }
    ...
    public digitalRead(pin: number): number | null {
        if (!this.initialized) {
            console.error(`[MCP23017] Device ${this.name} not initialized. Cannot read pin.`);
            return null;
        }
        ...
        try {
            ...
            console.error(`[MCP23017] digitalRead error on pin ${pin}: ${err.message}`);
            return null;
        }
    }
    ...
    public readAll(): number | null {
        if (!this.initialized) {
            console.error(`[MCP23017] Device ${this.name} not initialized. Cannot read all pins.`);
            return null;
        }
        try {
