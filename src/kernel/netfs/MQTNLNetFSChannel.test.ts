...
import { NetFSError, NETFS_VERSION, encodeNetFSRequest, encodeNetFSResponse } from "../../common/netfs/NetFSProtocol";
...
describe("MQTNLNetFSChannel (N4)", () => {
    ...
    beforeEach(() => {
        ...
        driver = new SimpleMQTNLDriver("smqtnl_netfs_test", "mqtt://127.0.0.1:1883", "tsix_netfs_test");
        ...
        (driver as any).send = async (address: string, port: number, data: any, flag: number, srcPort: number) => {
    ...
    it("N4.02 send() mengirim frame biner sebagai paket DATA dari port lokal mount", async () => {
        const ch = MQTNLNetFSChannel.open(kernel, {
            address: "tsix_2",
            port: 7777,
        });

        const frame = encodeNetFSRequest({ id: 1, op: "info" });
        const ok = await ch.send(frame);
        ...
        // Payload biner diteruskan utuh — bukan string/JSON yang di-encode ulang.
        expect(sent[0].data).toBe(frame);
    ...
    it("N4.03 balasan dari driver diteruskan ke driver NetFS", () => {
        ...
        const payload = encodeNetFSResponse({ v: NETFS_VERSION, id: 1, ok: true, result: null });
        handlers.get(ch.port)!({ src: "tsix_2", port: 7777, localPort: ch.port, data: payload });
    ...
    it("N4.04 close() melepas port + handler, dan send ditolak setelahnya", async () => {
        ...
        expect(await ch.send(Buffer.from("{}"))).toBe(false);
    ...
    it("N4.05 interface tidak dikenal → NetFSError EIO (mount gagal cepat)", () => {
        ...
        expect(() => MQTNLNetFSChannel.open(empty, { address: "tsix_2", port: 7777, iface: "smqtnl99" })).toThrow(
            NetFSError,
        );
    });
    ...
    it("N4.06 key mengaktifkan enkripsi pada port lokal mount", () => {
        ...
        // 2 ioctl: pin protocol Binfeo per-port (selalu) + upgrade security (bila key).
        expect(ioctls).toHaveLength(2);
        expect(ioctls[0]).toMatchObject({ cmd: 0x1002, port: ch.port, protocol: "Binfeo" });
    ...
    it("N4.08 open() mem-pin protocol Binfeo per-port, close() melepasnya", async () => {
        // Regresi konflik framing: port channel TIDAK boleh mewarisi protocol dari
        // `protocolRegistry`/`activeProtocol` — NetFS v2 selalu BINER via Binfeo.
        const ch = MQTNLNetFSChannel.open(kernel, { address: "tsix_2", port: 7777 });

        expect(ioctls).toEqual([{ cmd: 0x1002, port: ch.port, protocol: "Binfeo" }]);
