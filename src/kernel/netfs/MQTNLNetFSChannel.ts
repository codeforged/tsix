...
import { NetFSError, NETFS_WIRE_PROTOCOL } from "../../common/netfs/NetFSProtocol";
...
export class MQTNLNetFSChannel implements INetFSChannel {
     ...
     * kernel sendiri.
     */
    public static open(kernel: KernelLike, opts: MQTNLNetFSChannelOptions): MQTNLNetFSChannel {
        ...
        // Protocol per-port DI-PIN — NetFS v2 selalu berbicara BINER lewat Binfeo.
        ...
        // pernah kejadian request NetFS ikut ter-frame Binfeo/JSON secara tidak
        // sengaja → payload sampai dalam bentuk yang salah → dibuang diam-diam
        // (mount hang sampai timeout). Pin ini membuat framing deterministik.
        driver.ioctl(0x1002, { port: localPort, protocol: NETFS_WIRE_PROTOCOL });
    ...
    private static resolveDriver(kernel: KernelLike, iface?: string): SimpleMQTNLDriver | null {
    ...
    /** send(): Kirim satu frame biner request ke peer lewat MQTNL. */
