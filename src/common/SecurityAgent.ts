 ...
 * Tetap mendukung XOR (Legacy) untuk kompatibilitas.
 */
export class SecurityAgent {
     ...
     */
    public static encryptWithPublicKey(publicKeyPem: string, data: Buffer): string {
     ...
     */
    public static decryptWithPrivateKey(privateKeyPem: string, hexData: string): Buffer {
    ...
    public setSessionKey(key: Buffer | string) {
        if (typeof key === "string") {
            ...
            if (buf.length !== 32) throw new Error("ChaCha20 requires a 32-byte session key (64 hex chars).");
            this.sessionKey = buf;
        } else {
            if (key.length !== 32) throw new Error("ChaCha20 requires a 32-byte session key.");
            this.sessionKey = key;
        }
    }
    ...
    public securePacketOutRaw(data: string | Buffer): Buffer | string {
        ...
        if (this.sessionKey) {
            ...
            const cipher = crypto.createCipheriv("chacha20-poly1305", this.sessionKey, iv, {
                authTagLength: 16
            });
    ...
    public securePacketInRaw(data: Buffer): string {
        ...
        try {
            if (this.sessionKey) {
                ...
                const decipher = crypto.createDecipheriv("chacha20-poly1305", this.sessionKey, iv, {
                    authTagLength: 16
                });
