...
describe("SyscallCode", () => {
  ...
  it("D2.01 no duplicate codes", () => {
    const codes = Object.values(SyscallCode).filter(
      (v) => typeof v === "number",
    ) as number[];
  ...
  it("D2.03 reverse mapping – code → name exists for all", () => {
    const entries = Object.entries(SyscallCode).filter(
      ([k, v]) => typeof v === "number",
    ) as [string, number][];
    // Build forward map
  ...
  it("D2.07 new codes don't overlap with existing", () => {
    ...
    const allCodes = Object.values(SyscallCode).filter(
      (v) => typeof v === "number",
    ) as number[];
  ...
  // ============================================================
  // D2.11: Runtime network default
  // ============================================================
  it("D2.11 SET_NET_DEFAULT = 76 exists", () => {
    expect(SyscallCode.SET_NET_DEFAULT).toBe(76);
  });
  ...
  it("D2.10 no gap in critical range (1–65 contiguous check)", () => {
    ...
    const codes = new Set(
      Object.values(SyscallCode).filter(
        (v) => typeof v === "number",
      ) as number[],
    );
