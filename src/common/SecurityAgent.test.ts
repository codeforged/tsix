...
describe("SecurityAgent", () => {
  ...
  it("D3.07 RSA – encrypt + decrypt roundtrip", () => {
    ...
    const decrypted = SecurityAgent.decryptWithPrivateKey(
      privateKey,
      encrypted,
    );
  ...
  it("D3.08 RSA – wrong private key fails decrypt", () => {
    ...
    const encrypted = SecurityAgent.encryptWithPublicKey(
      publicKey,
      Buffer.from("test"),
    );
    expect(() =>
      SecurityAgent.decryptWithPrivateKey(wrongKey, encrypted),
    ).toThrow();
  });
