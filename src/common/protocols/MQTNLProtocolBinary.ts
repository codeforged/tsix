...
export class MQTNLProtocolBinary implements IMQTNLProtocol {
  /** Magic byte penanda protocol (ditimpa subclass utk protocol turunan). */
  protected magicByte: number = 0x42; // 'B'
  /** Version wire protocol (ditimpa subclass utk protocol turunan). */
  ...
  pack(packet: any): Buffer {
    ...
    const headerSize =
      2 + 1 + srcAddr.length + 2 + 1 + dstAddr.length + 2 + 2 + 2 + 4 + 1 + 1;
    const buf = Buffer.alloc(headerSize + payload.length);
    let offset = 0;
    buf.writeUInt8(this.magicByte, offset++); // Magic
    buf.writeUInt8(this.protoVersion, offset++); // Proto Ver
  ...
  unpack(data: Buffer | string): any {
    ...
    let offset = 0;
    if (buffer.readUInt8(offset++) !== this.magicByte)
      throw new Error("Invalid Magic Byte");
    if (buffer.readUInt8(offset++) !== this.protoVersion)
      throw new Error("Invalid Protocol Version");

    const srcAddrLen = buffer.readUInt8(offset++);
    const srcAddress = buffer
      .subarray(offset, offset + srcAddrLen)
      .toString("utf8");
    offset += srcAddrLen;
    ...
    const dstAddress = buffer
      .subarray(offset, offset + dstAddrLen)
      .toString("utf8");
    offset += dstAddrLen;
