        ...
        const syncDir = (hostDir: string, vfsDir: string) => {
            ...
            for (const item of items) {
                ...
                if (stats.isDirectory()) {
                    ...
                    // (1 byte = 1 char) — cocok dengan Buffer.from(raw,"latin1")
                    // di sisi app. Teks biasa pakai utf8.
                    const isBinary =
                    ...
                    if (fullVfsPath.endsWith(".ts")) {
                        try {
                            ...
                            console.error(`[VFS-Bootstrap]   -> \x1b[1;33mWarning: Compilation failed for ${fullVfsPath}: ${e.message}\x1b[0m`);
                        }
                    }

                    // Auto-executable untuk file di direktori eksekusi
                    if ((isSetuidBinary(fullVfsPath) || isExecutableBinary(fullVfsPath)) && (fullVfsPath.endsWith(".ts") || fullVfsPath.endsWith(".js"))) {
