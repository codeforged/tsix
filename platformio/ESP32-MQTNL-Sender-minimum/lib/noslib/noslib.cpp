...
NOS::NOS(const char *id, const uint16_t port, const char *apiKey, const char *mqttServer, int mqttPort)
    : id(id), port(port), apiKey(apiKey), mqttServer(mqttServer), mqttPort(mqttPort), mqttClient(espClient)
{
    // API key dikirim sebagai string hex (64 char) dari main.cpp;
    // konversi ke 32 byte mentah untuk kunci ChaCha20.
    memset(keyBytes, 0, sizeof(keyBytes));
    hexToBytes(apiKey, keyBytes, sizeof(keyBytes));
}

bool NOS::hexToBytes(const char *hex, uint8_t *out, size_t outLen)
{
    if (!hex)
        return false;
...
void NOS::mqttCallback(char *topic, byte *payload, unsigned int length)
{
    ...
    if (message)
    {
        ...
        if (hexLen < 24 + TAG_SIZE * 2)
            return; // Validasi minimum: nonce (12 byte) + tag (16 byte) = 28 byte (56 hex char)
...
void NOS::encryptData(const uint8_t *input, uint8_t *output, const uint8_t *vnonce, size_t length)
{
    ...
    // Enkripsi
    chacha.clear();
    chacha.setKey(keyBytes, KEY_SIZE);
