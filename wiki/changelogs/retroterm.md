...
### Profil warna tabung: `green` / `amber` / `color`

- **File:** `src/mirror/opt/retroterm/retroterm.ts`, `src/mirror/opt/dome/dome-client-term.js`, `src/mirror/opt/asteracea/menu/retroterm.menu`
- **Permintaan:** warna fosfor hijau sudah ada; tambahkan profil **amber** dan **berwarna**, dan cukup dipilih lewat parameter — mis. `retroterm green|amber|color`.
- **Perubahan:**
    - Konstanta `PHOSPHOR` diganti **`PROFILES`** (`CrtProfile`): tiap profil membawa `screenBg`, `tint`, `selection`, `specular`, dan palet xterm lengkap (18 warna ANSI + `foreground`/`cursor`).
    - **Tiga profil:**
      | Profil | Karakter | `screenBg` | `tint` | foreground |
      | --- | --- | --- | --- | --- |
      | `green` (default) | fosfor hijau P1 — **nilai lama, tidak diubah** | `#03130a` | `rgba(0,255,120,0.045)` | `#33ff66` |
      | `amber` | fosfor amber P3, map ANSI → gradasi amber | `#170d02` | `rgba(255,170,0,0.05)` | `#ffb000` |
      | `color` | CRT berwarna, palet ANSI normal | `#04070c` | `""` (tanpa tint) | `#cccccc` |
    - **Pemilihan profil:** bare argumen di posisi bebas (`retroterm amber`), atau eksplisit `--profile amber` / `--theme amber` / `-p amber` (termasuk bentuk `--theme=amber`). Profil **tidak dikenal** → peringatan + fallback ke `green` (tidak diam-diam diabaikan).
    - **Judul window** menampilkan profil **hanya bila diminta eksplisit** (`RetroTerm [amber]`) supaya deteksi command di title bar & tampilan default tetap sama.
    - **`cmdArg` tidak lagi menangkap nama profil** — `retroterm amber` dulu akan mengirim string `"amber"` ke shell; kini nama profil dikeluarkan dari pemilihan command (`retroterm amber /bin/ls.js` benar).
    - **`applyCrtFx()` menerima `crt.specular`** (`{ top, bottom }` = `"r,g,b"`) untuk warna kilau kaca cembung. Sebelumnya warna kilau **hardcoded hijau** (`190,255,210` / `120,255,170`) sehingga profil amber/berwarna akan tetap berpendar hijau. Default di sisi browser = nilai lama → **tanpa regresi** untuk pemanggil yang tidak mengirim `specular`.
    - Tambah **`--help` / `-h`** (`HELP`) — daftar profil, opsi, dan contoh; plus komentar profil di `retroterm.menu`.
- **Verifikasi:** harness Node dengan stub `@tsix/Application`/`emerald`/`theme` (7 kasus argumen) — semua **PASS**:
  `--profile bogus` → warning + `green`; `--profile amber` / `amber` / `--theme=amber` → amber; `color` → tint `""`; `-p green` → hijau eksplisit; `-hue` → hijau default. Tiap profil menghasilkan **21 kunci theme xterm** (tidak ada yang hilang) dan `screenBg` yang benar di `crtTheme` **maupun** latar container. `node --check` pada `dome-client-term.js` + `tsc --noEmit` bersih.
- **Deploy:** `npm run vfs:bootstrap` (app + klien DOME + menu). Tidak menyentuh kernel.
- **Oleh:** Copilot
