...
- **Masalah:** Di Tabulator v6, event klik-kanan baris bernama **`rowContext`** — bukan `rowContextMenu` (itu nama _option_ `contextMenu`, bukan event). Akibatnya listener tidak pernah terpanggil → `tb_contextmenu` tidak terkirim ke app → `showContextMenu()` di taskmgr tidak pernah dieksekusi → klik kanan tidak memunculkan menu.
