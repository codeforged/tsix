...
- **Hotkey** Alt+F1–F6 → `TTYManager.switch`. Pindah TTY juga bisa dengan **Alt+1–6** dan **Ctrl+Alt+1–6** di semua terminal (keduanya terkirim sebagai ESC+digit; di macOS Terminal: `¡ ™ £ ¢ ∞ §`) serta **Ctrl+1–6** di terminal yang melaporkan modifier (CSI-u / modifyOtherKeys); `Ctrl+4–6` juga terbaca lewat caret notation (FS/GS/RS). Tidak ada `Ctrl+1–6` telanjang: GNOME Terminal memakainya untuk pindah tab dan VS Code untuk focus editor group, dan byte-nya tak lagi membawa info modifier.
...
### Hotkey pindah TTY (Alt+F1..6, Alt+1..6, Ctrl+Alt+1..6) (`src/kernel/Kernel.ts`)
...
private handleKeyboardHotkey(seq: string): boolean {
    const hotkeys: Record<string, number> = {
        ...
        // Option+1..6 (macOS Terminal, Option as Meta MATI)
        "¡": 1, "™": 2, "£": 3, "¢": 4, "∞": 5, "§": 6,
    ...
    // typeof check: kata seperti "constructor"/"toString" jangan ikut ditelan
